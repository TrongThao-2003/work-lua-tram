...
function nguhanh()
	...
	local tbOpt =
	{
        ...
        "Chuyªn ®æi sang hÖ Kim/#convertSeries(0)",
        "Chuyªn ®æi sang hÖ Méc/#convertSeries(1)",
        "Chuyªn ®æi sang hÖ Thñy/#convertSeries(2)",
        "Chuyªn ®æi sang hÖ Háa/#convertSeries(3)",
        "Chuyªn ®æi sang hÖ Thæ/#convertSeries(4)",
...
function convertSeries(index)
    if(type(index)~="number" or index<0 or index>4)then
        Talk(1,"","L¹i sai!")
        return
    end
    if(GetSeries()==index)then
        Talk(1,"","Ng­¬i ®· lµ ngò hµnh"+GetSeriesName(index))
        return
    end
    SetSeries(index)
    Talk(1,"","§· chuyÓn ngò hµnh thµnh c«ng!")
    KickOutSelf()
end
function chuyengioi()
	if GetSex() == 1 then
