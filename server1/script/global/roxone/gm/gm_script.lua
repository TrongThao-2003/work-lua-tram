----------------------------------------------------
-- Script GM Vo Lam Truyen Ky OffLine 2025 Edit
----------------------------------------------------
IL("TITLE");
IncludeLib("TASKSYS")
IncludeLib("PARTNER");
...
function R_Action:showMe()
    ...
    return format(self.TITLEDIALOG, GetName(), GetAccount(), nW, nX, nY);
end
----------------------------------------------------------------------------------
-- NhËp mËt khÈu tr­íc khi sö dông ®­îc chøc n¨ng GM
----------------------------------------------------------------------------------
function R_Action:GMPassword()
    ...
    if (GetLevel() < 10) then
        Talk(1, "", "§Ó sö dông ®­îc toµn bé chøc n¨ng bªn trong, vui lßng n©ng lªn <color=red>cÊp 10<color> råi h·y sö dông ®?n n?");
        return 0
    end
...
function R_Action:showMain()
    local tbSay = self:showMe()
    local tbOption = {}
    tinsert(tbOption, { "Qu¶n lý nh©n vËt", self.R_PlayerManager, { self } });
...
function R_Action:EnterGMPassword(nIdx, szAccount, szName, szPassword)
    ...
    Msg2Player("B¹n ®· nhËp mËt khÈu ®óng, ®¨ng nhËp thµnh c«ng! B©y giê c? thÓ sö dông ®­îc chøc n¨ng dµnh cho GM.")
end
--------------------------------------------
--                 GM Menu               --
--------------------------------------------
function R_Action:R_PlayerManager()
    local tbSay = self:showMe()
    local tbOption = {}
    tinsert(tbOption, { "Phôc håi", R_restore });
    tinsert(tbOption, { "NhËn Point", CacLoaiDiem });
    tinsert(tbOption, { "Hæ trî lµm nhiÖm vô", R_help_nv });
    tinsert(tbOption, { "Thay §æi Tr¹ng Th¸i", trangthai });
    tinsert(tbOption, { "Söa lçi ThÇn Hµnh Phï", fixthanhanhphu });
    tinsert(tbOption, { "NhËn Vßng S¸ng", R_GetLight});
    tinsert(tbOption, { "Quay l¹i", self.showMain, { self } });
    CreateNewSayEx(tbSay, tbOption)
end
function R_Action:ManagePlayerSystem()
    local szTitle = self:showMe()
    local tbOption = {}
    tinsert(tbOption, { "Run Tong Kim Test", function()
    ...
    tinsert(tbOption, { "Reload Files", self.Reloadfiles })
    tinsert(tbOption, { "Quay l¹i.", self.showMain, { self } })
    CreateNewSayEx(szTitle, tbOption)
end
function R_Action:ManagePlayerAccountSystem()
    local szTitle = self:showMe()
    local tbOption = {}
...
function R_Action:DiChuyenGM()
    local szTitle = self:showMe()
    local tbOption = {};
    tinsert(tbOption, { "LÊy täa ®é map", self.R_getPosWorld, { self } })
    tinsert(tbOption, { "DÞch chuyÓn tøc thêi", self.MoveToPosition, { self } })
    tinsert(tbOption, { "§Õn ®Þa ®iÓm kh¸c", self.GM_dichuyen, { self } })
    tinsert(tbOption, { "Quay l¹i.", self.showMain, { self } })
    CreateNewSayEx(szTitle, tbOption)
end
function R_Action:TinhNangGM()
    local szTitle = self:showMe()
    local tbOption = {}
    tinsert(tbOption, { "LÊy trang bÞ Hoµng Kim", self.GetItemGold, { self } })
    tinsert(tbOption, { "LÊy trang bÞ B¹ch Kim", self.GetItemPlatina, { self } })
    tinsert(tbOption, { "LÊy Item by ID", self.GetItembyId, { self } })
    tinsert(tbOption, { "Dän s¹ch r­¬ng ®å", self.DonSachRuong, { self } })
    tinsert(tbOption, { "NhËn 1000 Hçn Nguyªn Linh Lé ", self.R_x2KN, { self } })
...
--------------------------------------------
-- Menu Qu¶n Lý Nh©n VËt       --
--------------------------------------------
function R_GetLight()
    g_AskClientStringEx("", 0, 256, "NhËp Sè VS", {R_GetLight_Check})
end
function R_GetLight_Check(szLightID)
    local nLightID = tonumber(szLightID)
    if (nLightID == nil or nLightID <= 0) then
        Msg2Player("ID vßng s¸ng kh«ng hîp lÖ, vui lßng kiÓm tra l¹i!")
        return
    end
    Title_AddTitle(szLightID, 1,24 * 60 * 60 * 18) -- 1 ngµy
    Title_ActiveTitle(szLightID)

    SetTask(1122, szLightID)
    Msg2Player("B¹n ®· nhËn ®­îc 1 vßng s¸ng!")
end
function R_help_nv()
    local tbSay = self:showMe()
    local tbOption = {}
    tinsert(tbOption, { "Hæ trî lµm nhiÖm vô Hoµng Kim", helpgoldquest });
    tinsert(tbOption, { "Hæ trî lµm nhiÖm vô M«n Ph¸i", help_quest });
    tinsert(tbOption, { " Boss", bosshk1 });
    tinsert(tbOption, { "Quay l¹i", self.showMain, { self } });
    CreateNewSayEx(tbSay, tbOption)
...
function trangthai()
    ...
    local tbOpt = {
        { "BËt chiÕn ®Êu", function()
            SetFightState(1)
        end },
        { "T¾t chiÕn ®Êu", function()
            SetFightState(0)
        end },
...
--------------------------------------------
-- Menu Qu¶n Lý M¸y Chñ
--------------------------------------------
function R_Action:Auto_TestBoss()
    -- Call Boss
    BossHK(f_bossx, f_bossy)
end
TBBOSSS = { --tªn boss,id boss, tû lÖ r¬i ®å,series boss,nLevel
    [1] = { szName = "HuyÒn Gi¸c §¹i S­ ssss", nBossId = 1111, nRate = 322, nSeries = 0, nLevel = 95 },
    ...
    [12] = { szName = "§­êng Phi Y?n", nBossId = 1366, nRate = 200, nSeries = 1, nLevel = 95 },
    ...
    [16] = { szName = "Gia LuËt T? Ly", nBossId = 563, nRate = 200, nSeries = 3, nLevel = 95 },
    ...
    [21] = { szName = "H¹t Huy Hoµng", nBossId = 1111, nRate = 200, nSeries = 0, nLevel = 95 },
    [22] = { szName = "Thanh Liªn Tö", nBossId = 1368, nRate = 200, nSeries = 4, nLevel = 95 },
...
function BossHK(f_bossx, f_bossy)
    ...
    local n_count = getn(TBBOSSS);
    local tb = {}
    tb = { GetName() .. " b¹n muèn th¶ boss nµo?" }
    for i = f_bossx1, f_bossy1 do
        tinsert(tb, format("%s/#PickBosss(%d)", TBBOSSS[i].szName, i));
    end
    if (f_bossx1 ~= 1) then
...
function PickBosss(nIndex)
    if GetFightState() == 0 then
        Talk(1, "", "Kh«ng thÓ th¶ boss ë nh÷ng n¬i phi chi?n ®Êu ®­îc.")
        return
    end
    local item = TBBOSSS[nIndex];
    local nw, nx, ny = GetWorldPos();
...
function R_Action:Reloadfiles()
    -- Reload file
    local tbSay = { "<color=yellow>GM:<color><enter>Danh s¸ch Script<enter>Link: <color=green>/script/global/GM/config/cfg_server.lua<color><enter>Link: <color=green>/script/global/GM/gm/gm_script.lua<color>" };
    tinsert(tbSay, "Reload Script CFG_server/Reload_CFG_server")
    tinsert(tbSay, "NhËp ®­êng dÉn Script/Reloadfile")
    tinsert(tbSay, "Quay l¹i/#self:showMain()" );
    CreateTaskSay(tbSay);
end

-----------------------------------------------------------
-- Menu Qu¶n Ly Ng­êi Ch¬i
-----------------------------------------------------------

-----------------------------------------------------------
-- Menu GM Di ChuyÓn
-----------------------------------------------------------
function R_Action:R_getPosWorld()
    local w, x, y = GetWorldPos();
    local nIdplayer = PlayerIndex;
    Say("Map ID <color=Green>" .. w .. "<color> täa ®é <color=Yellow> " .. x .. ", " .. y);
end
function R_Action:MoveToPosition()
    g_AskClientStringEx(GetStringTask(TASK_S_POSITION), 0, 256, "ID=53,X=200,Y=200", { self.EnterPosition, { self } })
end

function R_Action:EnterPosition(szPos)
    ...
    if nMove ~= 1 then
        GMMsg2Player("D?ch chuyÓn ®?n v? tr?", "ThÊt b¹i! V? tr? kh«ng hîp lÖ, vui lßng kiÓm tra l¹i.")
        return
    end
...
function go_PLD()
    Say("<color=yellow>GM:<color><enter>Mêi GM <color=yellow>" .. GetName() .. "<color> chän n¬i ngµi muèn ®Õn.", 7,
...
function MapCTC()
    Say("ThÇn hµnh phï c? thÓ ®­a ®¹i hiÖp <color=yellow>" .. GetName() .. "<color> ®?n bÊt cø thµnh th? hay th«n trÊn nµo, vui lßng chän n¬i ngµi muèn ®?n.", 8,
            "GM go: Chi?n tr­êng biÖn kinh/MapBK",
            "GM go: Chi?n tr­êng l©m an/MapLA",
            "GM go: Chi?n tr­êng ph­îng t­êng/MapPT",
            "GM go: Chi?n tr­êng t­¬ng d­¬ng/MapTD",
            "GM go: Chi?n tr­êng thµnh ®«/MapTDo",
            "GM go: Chi?n tr­êng d­¬ng ch©u/MapDC",
            "GM go: Chi?n tr­êng ®¹i l?/MapDL",
            "Rêi khái/no"
    );
end
...
-- Menu GM Chøc N¨ng
----------------------------------------------------------
function R_Action:R_x2KN()
    for i = 1, 1000 do
        AddItem(6, 1, 2312, 1, 0, 0, 0)
    end
    Talk(1, "", "NhËn thµnh c«ng 1000 hçn nguyªn linh lé")
end
function R_Action:GetItembyId()
    g_AskClientStringEx(GetStringTask(R_GETITEM), 0, 256, "NhËp ID", { self.R_getItem, { self } })
end;
function R_Action:GetItemGold()
    g_AskClientStringEx("", 0, 256, "NhËp ID", { self.R_getItemGold, { self } })
end;
function R_Action:GetItemPlatina()
    g_AskClientStringEx("", 0, 256, "NhËp ID", { self.R_getItemPlatina, { self } })
end;
function R_Action:R_getItemPlatina(szItem)
    if not szItem then
        Talk(1, "", "Ch­a nhËp th«ng sè cña mét item bÊt kú, vui lßng thö l¹i.")
        return 0
    end
    local tbItem = split(szItem, ",")
    local x = tonumber(tbItem[1])
    local y = tonumber(tbItem[2])
    if not x or not y then
        Talk(1, "", "Ch­a nhËp ®óng th«ng sè,ph¶i lµ <color=Yellow> value1,value2 <color>")
    end
    lAddPlatinaItem(x, y)
    Talk(1, "", " <color=Green>LÊy vËt phÈm hoµng kim thµnh c«ng<color>")
end
function R_Action:R_getItemGold(szItem)
    if not szItem then
        Talk(1, "", "Ch­a nhËp th«ng sè cña mét item bÊt kú, vui lßng thö l¹i.")
        return 0
    end
...
function R_Action:R_getItem(szItem)
    if not szItem then
        Talk(1, "", "Ch­a nhËp th«ng sè cña mét item bÊt kú, vui lßng thö l¹i.")
        return 0
    end
    local tbItem = split(szItem, ",") --6,1,71
    local x = tonumber(tbItem[1])
    local y = tonumber(tbItem[2])
    local z = tonumber(tbItem[3])
    local nCount = tonumber(tbItem[4])
    if not nCount or nCount < 1 then
        nCount = 1
    end
    if not x or not y or not z then
        Talk(1, "", "Ch­a nhËp ®óng th«ng sè,ph¶i lµ <color=Yellow> value1,value2,value3 <color>")
    end
    for i = 1, nCount do
        AddItem(x, y, z, 1, 0, 0, 0)
    end
    Talk(1, "", " <color=Green>LÊy vËt phÈm thµnh c«ng<color>")
end
function R_Action:DonSachRuong()
    Say(" Dän!")
    local tbItems = {}
    tbItems = GetRoomItems(0);
    if (getn(tbItems) > 0) then
        for i = 7, getn(tbItems) do
            RemoveItemByIndex(tbItems[i], -1);
        end
    end
    self:GMLoginInGame()
end
function R_Action:TakeSpecifiedItem()
    g_AskClientStringEx(GetStringTask(TASKS_GETITEM), 0, 256, "Th«ng sè item:", { self.TakeSpecifiedItemParam, { self } })
end
function R_Action:TakeSpecifiedItemParam(szParam)
    if not szParam then
    ...
    if (nType == 0) then
        GMMsg2Player("LÊy vËt phÈm chØ ®?nh", "ThÊt b¹i! Tèi ®a cho phÐp lµ 16 th«ng sè.")
        return 0
    end
    local nIsParam = self:IsParamNumber(tbParam[2])
    if nIsParam ~= 1 then
        GMMsg2Player("LÊy vËt phÈm chØ ®?nh", "ThÊt b¹i! ChØ sö dông c¸c ki tù sè t? 0-9 vµ dÊu phÈy ?,?.")
        return 0
    end
    self:SetCountSpecifiedItem(nType, tbParam)
end
function R_Action:SetCountSpecifiedItem(nType, tbParam)
...
function R_Action:CountSpecifiedItem(tbSpecifiedItem, nCount, nOptionSelected)
    local szTitle = "H·y thi?t lËp thªm option n?u muèn.\n\n* Option GM ®· chän:"
    ...
    if nBind ~= 1 then
        tinsert(tbOpt, { "+ Thªm option b¶o hiÓm vÜnh viÔn", self.AddOptionInItem, { self, tbSpecifiedItem, nCount, 1 } })
        tinsert(tbOpt, { "+ Thªm option kh?a b¶o hiÓm", self.AddOptionInItem, { self, tbSpecifiedItem, nCount, 2 } })
    end
...
function R_Action:CreateItemSpecified(tbSpecifiedItem, nCount)
...
function R_Action:GetParamItem(tbSpecifiedItem)
...
function R_Action:NewItemEx(tbItem)
    return NewItemEx(
            tbItem[1],
            0,
            tbItem[2],
...
function R_Action:ReselectOption(tbSpecifiedItem, nCount)
    tbSpecifiedItem[3] = {}
    self:CountSpecifiedItem(tbSpecifiedItem, nCount)
end
function R_Action:GetTypeOption(tbOption, nType)
...
function R_Action:AddOptionInItem(tbSpecifiedItem, nCount, nOptionSelected)
    local tbListOption = {
        [1] = { "Kh?a b¶o hiÓm vÜnh viÔn", 1 },
        [2] = { "Kh?a b¶o hiÓm", 1 },
...
function R_Action:SetTimeInItem(tbSpecifiedItem, nCount, nOptionSelected, tbListOption)
...
function R_Action:SetTimeOptionInItem(tbSpecifiedItem, nCount, nOptionSelected, tbListOption, nTimer)
    tbSpecifiedItem[3][nOptionSelected] = { tbListOption[1], nTimer, tbListOption[2] };
    self:CountSpecifiedItem(tbSpecifiedItem, nCount)
end
function R_Action:IsOption(tbOpt, nType)
...
function R_Action:GetTypeParam(szParam)
    local nTypeParam = 0;
    local nMaxParam = 16;
...
function R_Action:IsParamNumber(tbParam)
...
------------------------------------------------------------
-- GM Qu¶n L? M¸y Chñ - Chøc n¨ng th«ng b¸o
------------------------------------------------------------
function R_Action:NotificationManage()
    local szTitle = "<color=yellow>GM:<color><enter>HiÖn t¹i c? 2 chøc n¨ng th«ng b¸o ch?nh, ®? lµ:\n+ <color=red>TÇng sè th? giíi<color>: kªnh nµy th«ng b¸o trªn khung ch¸t t¸n gÉu trong giao diÖn ng­êi ch¬i.\n+ <color=red>TÇng sè m¸y chñ<color>: kªnh nµy th«ng b¸o ®?n ng­êi ch¬i trong toµn m¸y chñ, néi dông ch¹y ch÷ ë ph?a trªn ®Çu nh©n vËt."
    local tbOption = {}
    tinsert(tbOption, { "Th«ng b¸o trªn tÇng sè th? giíi", g_AskClientStringEx, { "", 0, 256, "Néi dung th«ng b¸o:", { self.EnterNotification, { self, 1 } } } })
...
function R_Action:EnterNotification(nType, szNotices)
...
-----------------------------------------------------------
-- GM Qu¶n L? M¸y Chñ - LÊy th«ng tin NPC
-----------------------------------------------------------
...
------------------------------------------------------------
function R_Action:ManageSystemGlobal()
    local szTitle = "HÖ thèng qu¶n l? toµn m¸y chñ ®­îc sö dông ®Ó kh?a, cÊm ch¸t, kick out,? ng­êi ch¬i trong m¸y chñ."
    local tbOption = {}
    tinsert(tbOption, { "Kho¸ nh©n vËt ®ang trùc tuyÓn", self.LockPlayerOnline, { self, 1 } })
    tinsert(tbOption, { "Më kho¸ nh©n vËt", self.UnlockPlayer, { self, 1 } })
    tinsert(tbOption, { "Kh?a tµi kho¶n ®ang trùc tuy?n", self. LockPlayerOnline, { self, 2 } })
    tinsert(tbOption, { "Më kh?a tµi kho¶n", self. UnlockPlayer, { self, 2 } })
    tinsert(tbOption, { "Quay l¹i", self.ManagePlayerAccountSystem, { self } })
    CreateNewSayEx(szTitle, tbOption)
end
------------------------------------------------------------
-- GM Qu¶n L? Ng­êi Ch¬i - Qu¶n l? nh©n vËt - kh?a nh©n vËt ®ang trùc tuy?n/kh?a tµi kho¶n ®ang trùc tuy?n
------------------------------------------------------------
function R_Action:LockPlayerOnline(nType)
    g_AskClientStringEx("", 1, 50, "Tªn muèn kh?a:", { self.EnterLockPlayerOnline, { self, nType } })
end
function R_Action:EnterLockPlayerOnline(nType, szPlayer)
...
function R_Action:LockSystemByNamePlayer(nPlayerIndex)
    g_AskClientStringEx("", 1, 500, "L? do b? kh?a:", { self.EnterLockSystemByNamePlayer, { self, nPlayerIndex } })
end
function R_Action:EnterLockSystemByNamePlayer(nPlayerIndex, szMsg)
    local nType = 1;
    ...
    PlayerIndex = nPlayerIndex;
    szPlayerName = GetName();
    Msg2Player(format("<color=yellow>Nh©n vËt nµy ®· b? kh?a, v× l? do: %s", szMsg));
    ...
    OnTimer()
    PlayerIndex = nGMPlayer;
    Msg2Player("B¹n ®· kh?a ng­êi ch¬i nµy thµnh c«ng!")
    SetStringTask(TASKS_LOCKSYSTEM, szPlayerName);
end
function R_Action:LockSystemByAccountPlayer(nPlayerIndex)
    g_AskClientStringEx("", 1, 500, "L? do b? kh?a:", { self.EnterLockSystemByAccountPlayer, { self, nPlayerIndex } })
end
function R_Action:EnterLockSystemByAccountPlayer(nPlayerIndex, szMsg)
    local nType = 2;
    self:FileSystem_LoadFile(TAB_LINKFILEDATA[nType][1])
    local szPlayerName = "";
    local szMsg = szMsg or "";
...
function R_Action:SearchAccount(szAccount)
...
function R_Action:GameServerKickOut(nPlayerIndex)
    ...
    if (nIsPlayer == 1) then
        Msg2Player("Nh©n vËt nµy cña b¹n hiÖn ®ang bÞ kho¸, kh«ng thÓ tham gia vµo game.")
        SetTaskTemp(TASKTEMP_KICKOUT, GetCurServerTime());
        SetTimer(1 * FRAME2TIME, TIMETASK_ID);
    end
    ...
    if (nIsAccount == 1) then
        Msg2Player("Tµi kho¶n nµy cña b¹n hiÖn ®ang b? kh?a, kh«ng thÓ tham gia vµo game.")
        SetTaskTemp(TASKTEMP_KICKOUT, GetCurServerTime());
        SetTimer(1 * FRAME2TIME, TIMETASK_ID);
    end
end
------------------------------------------------------------
-- GM Qu¶n L? Ng­êi Ch¬i - Qu¶n l? nh©n vËt - Më kh?a nh©n vËt/Më kh?a tµi kho¶n
------------------------------------------------------------
function R_Action:UnlockPlayer(nType)
    g_AskClientStringEx("", 1, 50, "Tªn më kh?a:", { self.EnterUnlockPlayer, { self, nType } })
end

function R_Action:EnterUnlockPlayer(nType, szPlayer)
    self:FileSystem_LoadFile(TAB_LINKFILEDATA[nType][1])
    ...
    self:FileSystem_SaveData(TAB_LINKFILEDATA[nType][1])
    Msg2Player(format("B¹n ®· më kh?a cho <color=yellow>%s<color>  thµnh c«ng!", szPlayer));
end
-----------------------------------------------------------
-- GM Qu¶n L? Ng­êi Ch¬i - NhËp tªn nh©n vËt
-----------------------------------------------------------
function R_Action:ManipulationOnPlayer()
    if (GetLevel() < 10) then
        Talk(1, "", "Nh©n vËt ®¹t cÊp 10 trë lªn míi c? thÓ sö dông t?nh n¨ng nµy")
        return
    end
...
function R_Action:SearchPlayerOnline(szPlayer)
    local GMPlayer = PlayerIndex;
    ...
    Describe(format(strfill_center(" Th«ng tin ng­êi ch¬i ", 50, "-") .. "\n"
            ...
            .. "\n+ §iÓm phóc duyªn: %d"
            .. "\n+ §iÓm uy danh: %d"
            .. "\n+ §iÓm t?ch l?y Tèng Kim: %d"
            .. "\n+ §iÓm t?ch l?y Liªn §Êu: %d"
    ...
    , 11,
            "GM di chuyÓn ®?n ng­êi ch¬i nµy/#R_Action:GMMoveToPlayer(" .. nPlayerIndex .. ")",
            "Ng­êi ch¬i nµy di chuyÓn ®?n GM/#R_Action:PlayerMoveToGM(" .. nPlayerIndex .. ")",
            "Ng­êi ch¬i nµy vµo nhµ lao/#R_Action:PlayerMoveToNhaLao(" .. nPlayerIndex .. ")",
            "TÆng ®iÓm cho ng­êi ch¬i nµy/#R_Action:GivePoints(" .. nPlayerIndex .. ")",
            "TÆng vËt phÈm, ®¹o cô, trang b? cho ng­êi ch¬i nµy/#R_Action:GiveItemForPlayer(" .. nPlayerIndex .. ")",
            "TÆng tiªn th¶o lé ®Æc biÖt cho ng­êi ch¬i nµy/#R_Action:GiveItemTTL(" .. nPlayerIndex .. ")",
            "TÆng ti?n v¹n (v¹n l­îng) cho ng­êi ch¬i ngµy/#R_Action:GiveCash(" .. nPlayerIndex .. ")",
            "TÆng ti?n ®ång cho ng­êi ch¬i ngµy/#R_Action:GiveCoin(" .. nPlayerIndex .. ")",
            "TÆng KNB cho ng­êi ch¬i ngµy/#R_Action:GiveKNB(" .. nPlayerIndex .. ")",
            "Më chøc n¨ng cho ng­êi ch¬i kh¸c/#R_Action:MoChucNang(" .. nPlayerIndex .. ")",
            "Quay l¹i/OnCancel")
end

-----------------------------------------------------------
-- GM Qu¶n L? Ng­êi Ch¬i - NhËp tªn nh©n vËt - GM di chuyÓn ®?n ng­êi ch¬i nµy
-----------------------------------------------------------
function R_Action:GMMoveToPlayer(nPlayerIndex)
    local nWorld, nX, nY;
    local szName = "";
    local GMPlayer = PlayerIndex;
    PlayerIndex = nPlayerIndex;
    ...
    if nWorldIdx ~= 1 then
        GMMsg2Player(szName, "D?ch chuyÓn ®?n ng­êi ch¬i nµy thÊt b¹i!")
        return 0
    end
    GMMsg2Player(szName, "<color=yellow>D?ch chuyÓn ®?n ng­êi ch¬i nµy thµnh c«ng!")
end
-----------------------------------------------------------
-- GM Qu¶n L? Ng­êi Ch¬i - NhËp tªn nh©n vËt - Ng­êi ch¬i nµy di chuyÓn ®?n GM
-----------------------------------------------------------
function R_Action:PlayerMoveToNhaLao(nPlayerIndex)
    local GMPlayer = PlayerIndex
    PlayerIndex = nPlayerIndex
    local nRes = GMNhaLao()
    if (nRes ~= 1) then
        SetTask(TASKS_WASTED,1)
        PlayerIndex = GMPlayer;
        GMMsg2Player("Th«ng b¸o trõng ph¹t", "Giam vµo nhµ lao ng­êi ch¬i nµy thÊt b¹i!")

    end
    GMMsg2Player("Th«ng b¸o trõng ph¹t", "B¹n ®· bÞ giam vµo nhµ lao!")
    PlayerIndex = GMPlayer;
    GMMsg2Player("Th«ng b¸o trõng ph¹t ", "<color=yellow>B¹n ®· triÖu tËp ng­êi ch¬i nµy thµnh c«ng!")
end
function R_Action:PlayerMoveToGM(nPlayerIndex)
    local nWorld, nX, nY;
    local szPlayerName = "";
    local szGMName = "";
    local GMPlayer = PlayerIndex;
    ...
    if nWorldIdx ~= 1 then
        PlayerIndex = GMPlayer;
        GMMsg2Player(szPlayerName, "D?ch chuyÓn ng­êi ch¬i nµy thÊt b¹i!")
        return 0
    end
...
-----------------------------------------------------------
function R_Action:GivePoints(nPlayerIndex)
...
-----------------------------------------------------------
function R_Action:SetLevelPlayer(nPlayerIndex, nLevel)
...
-----------------------------------------------------------
function R_Action:SetExpPlayer(nPlayerIndex, nExp)
    local szPlayerName, szGMName = "", "";
    local nGMPlayer = PlayerIndex;
    szGMName = GetName();
    PlayerIndex = nPlayerIndex;
...
-----------------------------------------------------------
function R_Action:SetReputePlayer(nPlayerIndex, nPoint)
    local szPlayerName, szGMName = "", "";
    local nGMPlayer = PlayerIndex;
    szGMName = GetName();
    PlayerIndex = nPlayerIndex;
...
-----------------------------------------------------------
function R_Action:SetFuYuanPlayer(nPlayerIndex, nPoint)
    local szPlayerName, szGMName = "", "";
    local nGMPlayer = PlayerIndex;
    szGMName = GetName();
    PlayerIndex = nPlayerIndex;
...
-----------------------------------------------------------
function R_Action:SetBattlePointPlayer(nPlayerIndex, nPoint)
    local szPlayerName, szGMName = "", "";
    local nGMPlayer = PlayerIndex;
    szGMName = GetName();
    PlayerIndex = nPlayerIndex;
...
-----------------------------------------------------------
function R_Action:SetLeaguePointPlayer(nPlayerIndex, nPoint)
    local szPlayerName, szGMName = "", "";
    local nGMPlayer = PlayerIndex;
    szGMName = GetName();
    PlayerIndex = nPlayerIndex;
...
-----------------------------------------------------------
-- GM Qu¶n L? Ng­êi Ch¬i - NhËp tªn nh©n vËt - TÆng vËt phÈm, ®¹o cô, trang b? cho ng­êi ch¬i nµy
-----------------------------------------------------------
function R_Action:GiveItemTTL(nPlayerIndex)
    local GMPlayer = PlayerIndex;
    PlayerIndex = nPlayerIndex;
    local tbAwardItem = { tbProp = { 6, 1, 1181, 1, 0, 0 }, nExpiredTime = 24 * 3600, nBindState = -2 }
    tbAwardTemplet:GiveAwardByList(tbAwardItem, "TÆng vËt phÈm tiªn th¶o lé cho ng­êi ch¬i");
    Msg2Player(format("<color=green>B¹n nhËn ®­îc tiªn th¶o lé t? GM"))
    PlayerIndex = GMPlayer;
    Msg2Player(format("<color=green>B¹n ®· tÆng ng­êi ch¬i tiªn th¶o lé"))
end
function R_Action:GiveItemForPlayer(nPlayerIndex)
    local GMPlayer = PlayerIndex;
    PlayerIndex = GMPlayer;
...
-----------------------------------------------------------
-- GM Qu¶n L? Ng­êi Ch¬i - NhËp tªn nh©n vËt - TÆng ti?n v¹n (v¹n l­îng) cho ng­êi ch¬i nµy
-----------------------------------------------------------
function R_Action:GiveCash(nPlayerIndex)
    g_AskClientNumberEx(1, 20000, "NhËp sè l­îng:", { self.GiveCashNow, { self, nPlayerIndex } })
end
function R_Action:GiveCashNow(nPlayerIndex, nCount)
...
-----------------------------------------------------------
-- GM Qu¶n L? Ng­êi Ch¬i - NhËp tªn nh©n vËt - TÆng ti?n ®ång cho ng­êi ch¬i nµy
-----------------------------------------------------------
function R_Action:GiveCoin(nPlayerIndex)
    g_AskClientNumberEx(1, 1000000, "NhËp sè l­îng:", { self.GiveCoinNow, { self, nPlayerIndex } })
end
function R_Action:GiveCoinNow(nPlayerIndex, nCount)
    local szPlayer, szGMName = "", "";
    local szGMName = GetName();
    local nGMPlayer = PlayerIndex;
    PlayerIndex = nPlayerIndex;
    ...
    Msg2Player(format("<color=green>B¹n nhËn ®­îc <color=yellow>%d<color> Ti?n §ång t? GM %s", nCount, szGMName))
    PlayerIndex = nGMPlayer;
    Msg2Player(format("<color=green>B¹n ®· tÆng ng­êi ch¬i %s <color=yellow>%d<color> Ti?n §ång.", szPlayer, nCount))
end
-----------------------------------------------------------
-- GM Qu¶n L? Ng­êi Ch¬i - NhËp tªn nh©n vËt - TÆng KNB cho ng­êi ch¬i nµy
-----------------------------------------------------------
function R_Action:GiveKNB(nPlayerIndex)
    g_AskClientNumberEx(1, 1000000, "NhËp sè l­îng:", { self.GiveKNBNow, { self, nPlayerIndex } })
end
function R_Action:GiveKNBNow(nPlayerIndex, nCount)
    local szPlayer, szGMName = "", "";
    local szGMName = GetName();
    local nGMPlayer = PlayerIndex;
    PlayerIndex = nPlayerIndex;
...
-----------------------------------------------------------
-- GM Qu¶n L? Ng­êi Ch¬i - NhËp tªn nh©n vËt - Më chøc n¨ng cho ng­êi ch¬i kh¸c
-----------------------------------------------------------
function R_Action:MoChucNang(nPlayerIndex)
    local szPlayerName, szGMName = "", "";
    local nGMPlayer = PlayerIndex;
    szGMName = GetName();
    PlayerIndex = nPlayerIndex;
    szPlayerName = GetName();
    R_Action:SupportGMTestGame()
end

function R_Action:Write()
...
----------------------------------------------------------
function R_Action:ChangeMask()
    ChangeOwnFeature(0, 0, 567);
    Msg2Player("Bi?n thµnh h×nh d¹ng GM");
end
function R_Action:RestoreMask()
    RestoreOwnFeature();
...
----------------------------------------------------------
-- GM Chøc N¨ng - T?nh n¨ng hç trî kh¸c
----------------------------------------------------------
----------------------------------------------------------
-- GM Chøc N¨ng - T×m Boss Hoµng Kim
----------------------------------------------------------
function R_Action:GMHelp()
    findgoldboss(1, 12)
end
-----------------------------------------------------------
-- GM Chøc N¨ng - Kü n¨ng
-----------------------------------------------------------
function R_Action:SkillsSystem()
    local szTitle = "<color=yellow><color=yellow>GM:<color><enter><color> HÖ thèng kü n¨ng bao gåm thªm kü n¨ng vµ x?a kü n¨ng, b¹n muèn sö dông hÖ thèng kü n¨ng nµo?"
    local tbOpt = {}
...
function R_Action:AddSkills(szSkills)
...
function R_Action:DeleteSkills(szSkills)
    local tbSkills, nCount, nStart, _, nEnd, _ = self:GetSplitSkills(szSkills)
    if nCount > 2 then
        GMMsg2Player("Xãa kü n¨ng", "NhËp th«ng sè b? lçi, chØ c? thÓ nhËp tèi ®a 2 th«ng sè trë xuèng.")
        return 0
    end
    for i = nStart, nEnd do
        DelMagic(i)
...
function R_Action:GetSplitSkills(szString)
...
-----------------------------------------------------------
-- GM Di ChuyÓn - §ªn b¶n ®å c«ng thµnh ChiÕn
-----------------------------------------------------------
...
function goto_ctc(nIndex)
    if nIndex == 1 then
        NewWorld(926, 1713, 3296)
    elseif nIndex == 2 then
        NewWorld(927, 1750, 3386)
    elseif nIndex == 3 then
        NewWorld(928, 1716, 3292)
    elseif nIndex == 4 then
        NewWorld(929, 1763, 3507)
    elseif nIndex == 5 then
        NewWorld(930, 1752, 3392)
    elseif nIndex == 6 then
        NewWorld(931, 1712, 3294)
    elseif nIndex == 7 then
        NewWorld(932, 1714, 3293)
    end
end
-----------------------------------------------------------
...
function Reload_CFG_server()
    local script_list = {
        "script/global/roxone/gm/gm_script.lua",
        "script/global/roxone/gm/gm_lib.lua",
        "script/global/eurofun/item/lenhbaitestserver.lua"
    }
    for i = 1, getn(script_list) do
        LoadScript(script_list[i]);
        Msg2Player("<color=green> Reload thµnh c«ng script <color> !<enter><color=yellow>" .. script_list[i] .. "<color>");
    end
    --LoadScript("script/global/GM/config/cfg_server.lua");
    --LoadScript("script/global/GM/npc/npcthunghiem.lua");
    --LoadScript("script/global/Â·ÈË_Àñ¹Ù.lua");
    --LoadScript("script/global/roxone/gm/gm_script.lua");
    --LoadScript("script/global/roxone/gm/gm_lib.lua");
    --LoadScript("script/global/eurofun/item/lenhbaigm.lua");
    --Msg2Player("<color=green>Reload thµnh c«ng script CFG server");
    --Msg2Player("<color=green>Reload thµnh c«ng script CFG server");
    --Msg2Player("<color=green>Reload thµnh c«ng script CFG server");
end
-----------------------------------------------------------
...
----------------------------------------------------
-- LÊy vËt phÈm chØ ®iÞnh GM login
----------------------------------------------------
function R_Action:GMLoginInGame()
    if (self:CheckGameMaster() == 2) then
        local nLevel = GetLevel();
        if nLevel < 10 then
            ST_LevelUp(10 - nLevel);
        end
        ...
        if HaveCommonItem(6, 1, 5122) < 1 then
            tbAwardTemplet:GiveAwardByList({ { szName = "LÖnh Bµi Test Server", tbProp = { 6, 1, 5122, 1, 0 }, nCount = 1, }, }, "npctest", 1);
        end
    end 
end

-- KiÓm tra GM (kiÓm tra xem tµi kho¶n, nh©n vËt nµy c? ph¶i lµ GM hay kh«ng?)
-- Gi¸ tr? tr¶ v?: [-2]: Lçi table - [0]: kh«ng ph¶i GM - [1]: tµi kho¶n lµ GM - [2]: tªn tµi kho¶n vµ nh©n vËt lµ GM
--	Gi¸ tr? thø 2: sè thø tù cña TK
function R_Action:CheckGameMaster()
...
function R_Action:StartGameServer()
...
--		+ szSection: "SECTION"
--		+ szKey: T? kh?a cÇn load
--		+ szValue: Gi¸ tr? cña t? kh?a ®?
function R_Action:FileSystem_SetData(szLinkFile, szSection, szKey, szValue)
    IniFile_SetData(szLinkFile, szSection, szKey, szValue)
end

function R_Action:FileSystem_SaveData(szLinkFile)
...
--		+ szSection: "SECTION"
--		+ szKey: T? kh?a cÇn load
function R_Action:FileSystem_GetData(szLinkFile, szSection, szKey)
    return IniFile_GetData(szLinkFile, szSection, szKey)
end

-- Load d÷ liÖu
--		+ szLinkFile: ®­êng dÉn file d¹ng "\\data\\log.txt"
function R_Action:FileSystem_LoadFile(szLinkFile)
...
--		+ szLinkFile: ®­êng dÉn file d¹ng "\\data\\log.txt"
--		+ szSection = "TABLE"
--> Gi¸ tr? tr¶ v?: Sè l­îng dßng, danh s¸ch table
function R_Action:FileSystem_GetCount(szLinkFile, szSection)
    local tbKey = {}
...
function R_Action:GetIP()
    local tbIP = split(GetIP(), " : ")
    return tbIP[1], tbIP[2]
end

function OnTimer()
    --SetFightState(0); --Thi?t lËp tr¹ng th¸i chi?n ®Êu
    --ForbidChangePK(1); --CÊm chuyÓn PK
    --SetPKFlag(2); --Thi?t lËp PK
    --SetChatFlag(1); --CÊm ch¸t
    --DisabledStall(1); --CÊm bµy b¸n
    --ForbitTrade(1); --CÊm giao d?ch
    --DisabledUseTownP(1); --CÊm sö dông THP
    --ForbidEnmity(1); --CÊm c?u s¸t
    --SetCreateTeam(0); --Thi?t lËp t¹o tæ ®éi
    ...
    Msg2Player("<color=cyan>B¹n cßn " .. nTimeNow .. " gi©y nöa sÏ b? hÖ thèng kick ra khái game.")
    if (nTimeNow == 0) then
...
TBBOSS = { --tªn boss,id boss, tû lÖ r¬i ®å,series boss,nLevel
    [1] = { szName = "HuyÒn Gi¸c §¹i S­", nBossId = 740, nRate = 322, nSeries = 0, nLevel = 25 },
    [2] = { szName = "§­êng BÊt NhiÔm", nBossId = 741, nRate = 336, nSeries = 1, nLevel = 95 },
    [3] = { szName = "B¹ch Doanh Doanh", nBossId = 742, nRate = 336, nSeries = 1, nLevel = 95 },
    [4] = { szName = "Thanh TuyÖt S­ Th¸i", nBossId = 743, nRate = 341, nSeries = 2, nLevel = 95 },
    [5] = { szName = "Yªn HiÓu Tr¸i", nBossId = 744, nRate = 336, nSeries = 2, nLevel = 95 },
...
----------------------------------------------------Main----------
function R_Action:Init()
    if (self:GMPassword() ~= 0) then
        self:showMain()
    end
end