...
Include("\\script\\nationalwar\\head.lua")
Include("\\script\\global\\roxone\\gm\\gm_lib.lua")
function main(sel)
    print("--------------------------------NPC Nha lao-----------------------------------"..GetTask(TASKS_WASTED))
   if(GetTask(TASKS_WASTED)~=0)then
       Say("Ph¸ game mµ ®ßi ra tï,Nhµ ng­¬i cã ®ang n»m m¬ ban ngµy kh«ng! ",0)
      return
   end
	local Uworld96 = GetTask(96)
	local PK_value = GetPK();
	if (Uworld96 == 1) then			-- Ì½¼àµÄ
		Say("Th¨m xong ch­a vËy? Mau ®i ®i! ",2,"C¶m ¬n vŞ ®¹i ca nµy, ta ®i ®©yyyyy /visit_leave","Xin ®¹i ca th­ th¶ cho chót ®·! Ta sÏ ®i ngay th«i /no")
--	elseif (Uworld96 == 0) then		-- ³ö´íÁË
--		Talk(1,"","ÀÎÍ·£ºÎ¹£¡ÄãÊÇÔõÃ´»ì½øÌìÀÎÖØµØÀ´µÄ£¿ÓĞÊ²Ã´»°È¥ÕÒGM½âÊÍ°É£¡")
	elseif (PK_value <= 0 ) then		-- Ï´ÇåÁËPKµÄ
        Talk(2,"PKclear","Mau thu xÕp hµnh lı ®i!  Ng­¬i cã thÓ ra khái ®©y råi! ","§a t¹ ®ai ca, vËy ta ®i ®©yyyyy! ")
	else									-- Ï´PKµÄ
		-- Ìì×ÓÏµÍ³£º´óÉâÌìÏÂ
