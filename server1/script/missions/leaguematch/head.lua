...
function wlls_award_pl(nLevel, nWin, nTie, nTotal)
    ...
    --²ÎÈü¼ÆÊı
    if (nWin > 0) then
        SetTask(WLLS_TASKID_WIN, GetTask(WLLS_TASKID_WIN) + nWin)
    end
    if (nTie > 0) then
        SetTask(WLLS_TASKID_TIE, GetTask(WLLS_TASKID_TIE) + nTie)
    end
...
function wlls_AddMatchCount(str_lgname, nCount)
    ...
    if LG_GetLeagueTask(n_lid, WLLS_LGTASK_TOTAL) >= 48 then
        --´óÓÚ48³¡µÄ»°£¬Ôö¼ÓÀ©Õ¹´ÎÊı£¬·ñÔòÔö¼ÓÒ»´Î²Î¼Ó´ÎÊı
