...
function R_Print(testCase)
    for i = 1, getn(testCase) do
        local title = format("----------------Item %d Value=%s ----------------", i, testCase[i])
        print(title)
    end
end
--Ö÷¶Ô»°¿ò
...
function wlls_getaward_rank()
    ...
    if (n_award) then
        ...
        if (nRank == 1) then
            -- Nh?n ???c h?ng nh?t, ?i?m danh v?ng liªn ??u t?ng 1
...
function wlls_want2create()
    local n_level = wlls_getcityinfo()
    local n_next = GetGlbValue(GLB_WLLS_NEXT)
    if (not wlls_cancreate(n_level)) then
        return
    end
...
function wlls_buy_honour()
    ...
    if (GetBoxLockState() == 0) then
        Sale(188, 11) -- ÉÌµêµÄÈÙÓş»ı·Ö±àºÅÎª11
    else
